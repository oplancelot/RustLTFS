...
impl ScsiInterface {
    ...
    fn read_blocks_direct(&self, block_count: u32, buffer: &mut [u8]) -> Result<u32> {
        ...
        #[cfg(windows)]
        {
            ...
            if result {
                ...
                if is_file_mark {
                    info!("✅ File mark detected via sense data - read {} blocks before mark", actual_blocks_read);
                    Ok(actual_blocks_read)
                } else {
                    warn!("❌ READ(6) command failed with sense: {}", self.parse_sense_data(&sense_buffer));
                    Err(crate::error::RustLtfsError::scsi(format!("Direct block read operation failed: {}", 
    ...
    /// 返回 (实际读取的块数, 是否遇到文件标记)
    fn analyze_read_sense_data(&self, sense_data: &[u8], requested_bytes: u32) -> Result<(u32, bool)> {
        ...
        info!("🔍 Sense analysis: SenseKey=0x{:02X}, ASC=0x{:02X}, ASCQ=0x{:02X}", sense_key, asc, ascq);
        ...
        info!("🔍 DiffBytes from sense data: {} (requested {} bytes)", diff_bytes, requested_bytes);
        ...
        let actual_blocks_read = if actual_bytes_read > 0 {
            ...
            if (actual_bytes_read % block_sizes::LTO_BLOCK_SIZE) > 0 { 1 } else { 0 }
        } else {
            0
        };
        
        info!("🔍 Calculated: {} bytes read = {} blocks", actual_bytes_read, actual_blocks_read);
        ...
        if is_file_mark {
            info!("🎯 File mark detected: Add_Key=0x{:04X} (ASC:0x{:02X}, ASCQ:0x{:02X})", add_key, asc, ascq);
        } else {
        ...
        if final_is_file_mark {
            info!("✅ Final determination: FILE MARK detected - {} blocks read before mark", actual_blocks_read);
        }
