...
impl LtfsIndex {
    ...
    pub fn from_xml(xml_content: &str) -> Result<Self> {
        ...
        // Pre-validate XML structure before parsing
        if let Err(validation_error) = Self::validate_xml_structure(xml_content) {
            return Err(validation_error);
        }
        
        let index: LtfsIndex = quick_xml::de::from_str(xml_content)
            .map_err(|e| crate::error::RustLtfsError::parse(
                format!("Failed to parse LTFS index XML: {} (XML size: {} bytes)", e, xml_content.len())
            ))?;
        
        // Post-validation of parsed index
        Self::validate_parsed_index(&index)?;
        
        info!("Successfully parsed LTFS index, version: {}, generation: {}, files: {}", 
            index.version, index.generationnumber, Self::count_files_in_index(&index));
    ...
    pub fn from_xml_streaming(xml_content: &str) -> Result<Self> {
        debug!("Parsing large LTFS index using streaming parser, length: {}", xml_content.len());
        
        // For very large indexes (>100MB), use streaming approach
        if xml_content.len() > 100_000_000 {
            warn!("Large XML detected ({} bytes), parsing may take time", xml_content.len());
            
            // Try chunked parsing approach
            match Self::parse_xml_in_chunks(xml_content) {
                Ok(index) => return Ok(index),
                Err(e) => {
                    warn!("Chunked parsing failed: {}, falling back to standard parsing", e);
                }
            }
        }
    ...
    fn validate_xml_structure(xml_content: &str) -> Result<()> {
        ...
        if !xml_content.contains("<ltfsindex") {
            return Err(crate::error::RustLtfsError::parse("Missing LTFS index root element"));
        }
        
        if !xml_content.contains("</ltfsindex>") {
            return Err(crate::error::RustLtfsError::parse("XML appears incomplete - missing closing tag"));
        }
    ...
    fn validate_parsed_index(index: &LtfsIndex) -> Result<()> {
        ...
        if index.generationnumber == 0 {
            return Err(crate::error::RustLtfsError::parse("Invalid generation number"));
        }
        
        // Validate root directory
        if index.root_directory.name != "" {
            return Err(crate::error::RustLtfsError::parse("Root directory must have empty name"));
        }
    ...
    fn validate_directory_structure(directory: &Directory) -> Result<()> {
        ...
        if directory.uid == 0 {
            return Err(crate::error::RustLtfsError::parse(
                format!("Directory '{}' has invalid UID 0", directory.name)
            ));
        }
        ...
        for file in &directory.contents.files {
            if file.name.is_empty() {
                return Err(crate::error::RustLtfsError::parse("File with empty name found"));
            }
            
            if !file_names.insert(&file.name) {
                return Err(crate::error::RustLtfsError::parse(
                    format!("Duplicate file name '{}' in directory '{}'", file.name, directory.name)
                ));
            }
        }
        ...
        for subdir in &directory.contents.directories {
            if subdir.name.is_empty() {
                return Err(crate::error::RustLtfsError::parse("Directory with empty name found"));
            }
            
            if !dir_names.insert(&subdir.name) {
                return Err(crate::error::RustLtfsError::parse(
                    format!("Duplicate directory name '{}' in directory '{}'", subdir.name, directory.name)
                ));
            }
            ...
            if file_names.contains(&subdir.name) {
                return Err(crate::error::RustLtfsError::parse(
                    format!("Name conflict: '{}' exists as both file and directory", subdir.name)
                ));
            }
    ...
    fn validate_file_extents(directory: &Directory) -> Result<()> {
        ...
        for file in &directory.contents.files {
            ...
            if file.length > 0 && file.extent_info.extents.is_empty() {
                return Err(crate::error::RustLtfsError::parse(
                    format!("File '{}' has size {} but no extents", file.name, file.length)
                ));
            }
            ...
            for extent in &file.extent_info.extents {
                ...
                if extent.partition != "a" && extent.partition != "b" && 
                   extent.partition != "A" && extent.partition != "B" {
                    return Err(crate::error::RustLtfsError::parse(
                        format!("Invalid partition '{}' in file '{}'", extent.partition, file.name)
                    ));
                }
                ...
                if extent.byte_count == 0 {
                    return Err(crate::error::RustLtfsError::parse(
                        format!("Zero-size extent in file '{}'", file.name)
                    ));
                }
                ...
                if extent.file_offset < last_file_offset {
                    return Err(crate::error::RustLtfsError::parse(
                        format!("Extents not ordered by file offset in file '{}'", file.name)
                    ));
                }
            ...
            if total_extent_size != file.length {
                return Err(crate::error::RustLtfsError::parse(
                    format!("File '{}': declared size {} doesn't match extent total {}", 
    ...
    fn validate_uid_uniqueness(index: &LtfsIndex) -> Result<()> {
        ...
        if !used_uids.insert(index.root_directory.uid) {
            return Err(crate::error::RustLtfsError::parse(
                format!("Duplicate UID {} found", index.root_directory.uid)
            ));
        }
        
        Self::collect_and_validate_uids(&index.root_directory, &mut used_uids)?;
        
        info!("UID validation passed, found {} unique UIDs", used_uids.len());
        Ok(())
    }
    
    /// Recursively collect and validate UIDs
    fn collect_and_validate_uids(directory: &Directory, used_uids: &mut std::collections::HashSet<u64>) -> Result<()> {
        // Check subdirectories
        for subdir in &directory.contents.directories {
            if !used_uids.insert(subdir.uid) {
                return Err(crate::error::RustLtfsError::parse(
                    format!("Duplicate UID {} found in directory '{}'", subdir.uid, subdir.name)
                ));
            }
        ...
        for file in &directory.contents.files {
            if !used_uids.insert(file.uid) {
                return Err(crate::error::RustLtfsError::parse(
                    format!("Duplicate UID {} found in file '{}'", file.uid, file.name)
                ));
            }
        }
    ...
    fn validate_timestamps(directory: &Directory) -> Result<()> {
        ...
        for file in &directory.contents.files {
            Self::validate_timestamp_format(&file.creation_time, &format!("file '{}' creation_time", file.name))?;
            Self::validate_timestamp_format(&file.change_time, &format!("file '{}' change_time", file.name))?;
            Self::validate_timestamp_format(&file.modify_time, &format!("file '{}' modify_time", file.name))?;
            Self::validate_timestamp_format(&file.access_time, &format!("file '{}' access_time", file.name))?;
            Self::validate_timestamp_format(&file.backup_time, &format!("file '{}' backup_time", file.name))?;
        }
    ...
    fn validate_timestamp_format(timestamp: &str, field_name: &str) -> Result<()> {
        ...
        if timestamp.len() < 20 { // Minimum: "2023-01-01T00:00:00Z"
            return Err(crate::error::RustLtfsError::parse(
                format!("Invalid timestamp format in {}: '{}' (too short)", field_name, timestamp)
            ));
        }
        
        if !timestamp.ends_with('Z') {
            return Err(crate::error::RustLtfsError::parse(
                format!("Invalid timestamp format in {}: '{}' (must end with Z)", field_name, timestamp)
            ));
        }
        
        if !timestamp.contains('T') {
            return Err(crate::error::RustLtfsError::parse(
                format!("Invalid timestamp format in {}: '{}' (missing T separator)", field_name, timestamp)
            ));
        }
        ...
        match chrono::DateTime::parse_from_rfc3339(timestamp) {
            ...
            Err(_) => {
                ...
                match chrono::DateTime::parse_from_str(timestamp, "%Y-%m-%dT%H:%M:%S%.fZ") {
                    Ok(_) => Ok(()),
                    Err(_) => Err(crate::error::RustLtfsError::parse(
                        format!("Invalid timestamp format in {}: '{}'", field_name, timestamp)
                    ))
                }
            }
        }
...
impl File {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.name.is_empty() {
            return Err(crate::error::RustLtfsError::parse("File name cannot be empty"));
        }
        
        if self.uid == 0 {
            return Err(crate::error::RustLtfsError::parse(
                format!("File '{}' has invalid UID 0", self.name)
            ));
        }
        ...
        if !self.is_symlink() {
            if self.length > 0 && !self.has_extents() {
                return Err(crate::error::RustLtfsError::parse(
                    format!("File '{}' has size {} but no extents", self.name, self.length)
                ));
            }
            ...
            for extent in &self.extent_info.extents {
                ...
                if extent.file_offset < last_offset {
                    return Err(crate::error::RustLtfsError::parse(
                        format!("File '{}': extents not properly ordered", self.name)
                    ));
                }
            ...
            if total_size != self.length {
                return Err(crate::error::RustLtfsError::parse(
                    format!("File '{}': size mismatch - declared: {}, extents total: {}", 
...
impl FileExtent {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        match self.partition.to_lowercase().as_str() {
            "a" | "b" => {},
            _ => return Err(crate::error::RustLtfsError::parse(
                format!("Invalid partition '{}'", self.partition)
            ))
        }
        
        // Check byte count
        if self.byte_count == 0 {
            return Err(crate::error::RustLtfsError::parse("Extent byte count cannot be zero"));
        }
        ...
        if self.byte_offset >= BLOCK_SIZE {
            return Err(crate::error::RustLtfsError::parse(
                format!("Byte offset {} exceeds block size {}", self.byte_offset, BLOCK_SIZE)
            ));
        }
...
impl LtfsIndex {
    ...
    pub fn extract_tape_file_locations(&self) -> Vec<TapeFileLocation> {
        ...
        info!("Extracted tape location information for {} files.", locations.len());
        locations
    }
    ...
    pub fn validate_for_tape_reading(&self) -> Result<()> {
        ...
        if self.location.partition.to_lowercase() != "a" {
            return Err(crate::error::RustLtfsError::parse(
                format!("索引必须存储在分区a，当前在分区{}", self.location.partition)
            ));
        }
        ...
        for location in &locations {
            if location.extents.is_empty() {
                return Err(crate::error::RustLtfsError::parse(
                    format!("文件 {} 缺少extent信息，无法从磁带读取", location.file_name)
                ));
            }
            
            // 验证extent总大小与文件大小一致
            let total_size: u64 = location.extents.iter()
                .map(|e| e.byte_count)
                .sum();
            if total_size != location.file_size {
                return Err(crate::error::RustLtfsError::parse(
                    format!("文件 {} 的extent总大小({})与文件大小({})不匹配", 
                            location.file_name, total_size, location.file_size)
                ));
            }
        }
        
        info!("Load格式索引验证通过，包含 {} 个可读取文件", locations.len());
        Ok(())
    }
    ...
    fn extract_locations_from_directory(&self, directory: &Directory, locations: &mut Vec<TapeFileLocation>) {
        // 处理当前目录的文件
        for file in &directory.contents.files {
    ...
    fn create_tape_location(&self, file: &File) -> TapeFileLocation {
        let extents: Vec<TapeDataExtent> = file.extent_info.extents.iter()
    ...
    fn search_file_in_directory(&self, directory: &Directory, target_path: &str) -> Option<TapeFileLocation> {
        // 在当前目录查找文件
        for file in &directory.contents.files {
